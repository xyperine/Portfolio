...
export class BeaconFactory {
	...
	createInteractParticles(projectData) {
		if (this.interactParticleSystems.has(projectData.interactive.number)) {
			return this.interactParticleSystems.get(
				projectData.interactive.number,
			);
		}
		...
		const ps = new QUARKS.ParticleSystem({
			...
			behaviors: [
				...
				new QUARKS.SizeOverLife(
					new QUARKS.PiecewiseBezier([
						[new QUARKS.Bezier(1, 1, 1, 0), 0],
					]),
				),
				new QUARKS.SpeedOverLife(
					new QUARKS.PiecewiseBezier([
						[new QUARKS.Bezier(1.0, 0.5, 0.25, 0), 0],
					]),
				),
			],
		});
		...
		let trailPs;
		if (
			this.interactTrailParticleSystems.has(
				projectData.interactive.number,
			)
		) {
			trailPs = this.interactTrailParticleSystems.get(
				projectData.interactive.number,
			);
		} else {
			trailPs = new QUARKS.ParticleSystem({
				...
				behaviors: [
					new QUARKS.SizeOverLife(
						new QUARKS.PiecewiseBezier([
							[new QUARKS.Bezier(1, 0.5, 0.25, 0), 0],
						]),
					),
					new QUARKS.SpeedOverLife(
						new QUARKS.PiecewiseBezier([
			...
			this.interactTrailParticleSystems.set(
				projectData.interactive.number,
				trailPs,
			);
		}
