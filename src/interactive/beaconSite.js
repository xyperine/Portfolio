...
export class BeaconSite {
	...
	createColumns() {
		for (let columnIndex = 0; columnIndex < 4; columnIndex++) {
			...
			let xPos =
				this.x + Math.sin(Math.PI * columnIndex * 0.5) * this.siteSize;
			let zPos =
				this.z + Math.cos(Math.PI * columnIndex * 0.5) * this.siteSize;
			...
			const rbd = RAPIER.RigidBodyDesc.fixed().setTranslation(
				xPos,
				yPos,
				zPos,
			);
	...
	createOrb() {
		...
		const orbRadius = 4;
		const sg = new THREE.SphereGeometry(
			orbRadius,
			orbRadius * 4,
			orbRadius * 4,
		);
				...
				THREE.UniformsLib.fog,
				{
					...
					uPrimaryColor: {
						value: new THREE.Color(
							this.projectData.interactive.color,
						),
					},
		...
		const srbd = RAPIER.RigidBodyDesc.fixed().setTranslation(
			sxPos,
			syPos,
			szPos,
		);
