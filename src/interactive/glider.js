...
export class Glider {
	...
	calculateLookRotation() {
		...
		this.pitchLookRotation = THREE.MathUtils.clamp(
			this.pitchLookRotation,
			pitchRange.bottom,
			pitchRange.up,
		);
		const pitchRotationSmoothing = 0.1;
		this.smoothedPitchRotation = THREE.MathUtils.lerp(
			this.smoothedPitchRotation,
			this.pitchLookRotation,
			pitchRotationSmoothing,
		);

		this.yawLookRotation += -mouseDelta.x * yawSpeed;
		const yawRotationSmoothing = 0.2;
		this.smoothedYawRotation = THREE.MathUtils.lerp(
			this.smoothedYawRotation,
			this.yawLookRotation,
			yawRotationSmoothing,
		);
	}
	...
	processPhysics() {
		...
		if (this.aboveSolidSurface) {
			this.yVelocity +=
				this.physicsWorld.gravity.y * this.physicsWorld.timestep;
			...
			if (error > 0) {
				...
				const acceleration =
					correctionAcceleration + dampingAcceleration;
		...
		const q = new THREE.Quaternion().setFromEuler(
			new THREE.Euler(0, this.smoothedYawRotation, 0),
		);
	...
	lookAt(position) {
		const direction = new THREE.Vector3()
			.subVectors(position, this.getRenderPosition())
			.normalize();
