...
export class PlanetNameGenerator {
	...
	generateSuffix(index) {
		...
		const availableIndexSchemes = [
			SCHEME.Letter,
			SCHEME.Numeral,
			SCHEME.RomanNumeral,
			SCHEME.Word,
		];
		let indexScheme = utils.seededRandomElement(
			this.random,
			availableIndexSchemes,
		);
		let suffix = "";
		switch (indexScheme) {
			case SCHEME.Letter: {
	...
	generateParentDesignation() {
		...
		const index = String(Math.round(this.random() * 10_000)).padStart(
			4,
			"0",
		);
	...
	generateDesignationSuffix(index) {
		...
		const indexScheme = utils.seededRandomElement(
			this.random,
			availableIndexSchemes,
		);
