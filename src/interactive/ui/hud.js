...
export class Hud {
	...
	constructor(glider, planetName, gravity) {
		...
		this.planetNameElement = document.querySelector(
			"#planet-name span:last-child",
		);
		this.coordElement = document.querySelector("#coord span:last-child");
		this.altitudeElement = document.querySelector(
			"#altitude span:last-child",
		);
		this.temperatureElement = document.querySelector(
			"#temperature span:last-child",
		);
		this.gravityElement = document.querySelector(
			"#gravity span:last-child",
		);
	}
	...
	update() {
		...
		this.temperatureElement.textContent = `${temperature.celsius.toFixed(1)}°C / \
