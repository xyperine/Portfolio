...
export class Compass {
    ...
    update(gameState) {
        const cameraHeading = this.getHeading(gameState.camera.getWorldDirection(this.lookDirection));
        ...
        for (let point of this.points) {
            const directionToPoint = (point.position.clone().sub(camPos)).normalize();
            const headingToPoint = this.getHeading(directionToPoint);
            let relativeHeadingToPoint = headingToPoint - cameraHeading;
            relativeHeadingToPoint = THREE.MathUtils.euclideanModulo(relativeHeadingToPoint + Math.PI, Math.PI * 2) - Math.PI;
            ...
            point.element.querySelector(".compass-pointer-distance").textContent = this.formatDistance(distance);
        }
    }
